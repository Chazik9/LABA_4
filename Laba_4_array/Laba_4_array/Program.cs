    ...
    internal class Program
    {
        ...
        static int[] DeleteElemet(int[] starList)//Функция которая Удаляет N элементов списка начинай с K-ого элемента
        {
            ...
            try
            {
                ...
                int K = CheckIntBigger0("Введите индекс элемента с которого надо удалить") - 1;//Запрашиваем K
        ...
        static int[] Exchange(int[] list)//Функция которая меняет Минимальный и Максимальные элементы списка местами 
        {
            try//Начало отслеживания ошибок
            {
                int maxIndex = 0, minIndex = 0;//индексы минимальных и максимальных значений списка
                for (int i = 0; i < list.Length; i++)//Перебор элементов списка
                {
                    if (list[i] > list[maxIndex])//Проверка на максимальный элемент
                    {
                        maxIndex = i;//присваиваем новый индекс
                    }
                    if (list[i] < list[minIndex])//Проверка на минимальный элемент
                    {
                        minIndex = i;//присваиваем новый индекс
                    }
                }
